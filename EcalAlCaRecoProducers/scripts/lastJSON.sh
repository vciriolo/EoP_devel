#!/bin/bash
jsonDir=/afs/cern.ch/cms/CAF/CMSCOMM/COMM_DQM/certification/Collisions17/13TeV/PromptReco
echo "Last Prompt JSON: "

ls -tr ${jsonDir}/*13TeV_PromptReco_Collisions17*_JSON*.txt | grep -v Muon |grep -v CaloOnly | grep -v ZeroTesla| grep -v LOWPU | grep -v NoL1T | tail -1
exit 0
